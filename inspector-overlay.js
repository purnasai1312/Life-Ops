/**
 * Inspector Overlay for LifeOps Preview
 *
...
(function () {
  ...
  // Prevent double-init
  if (window.__lifeopsInspector) return;
  ...
  overlay.id = '__lifeops-inspector-overlay';
  ...
  label.id = '__lifeops-inspector-label';
  label.style.cssText =
  ...
  var marginOverlays = ['top', 'right', 'bottom', 'left'].map(function (side) {
    ...
    el.id = '__lifeops-inspector-margin-' + side;
    el.style.cssText =
  ...
  var paddingOverlays = ['top', 'right', 'bottom', 'left'].map(function (side) {
    var el = document.createElement('div');
    el.id = '__lifeops-inspector-padding-' + side;
    el.style.cssText =
      'position:fixed;z-index:99998;pointer-events:none;display:none;' +
  ...
  function showGapOverlays(el) {
    ...
    for (var j = 0; j < children.length - 1; j++) {
      ...
      gapEl.className = '__lifeops-inspector-gap';
    ...
    try {
      if (window.parent && window.parent !== window) {
        window.parent.postMessage(
          { type: 'lifeops-inspector', action: action, payload: payload || null },
          '*'
        );
      }
    } catch (e) {
  ...
  window.addEventListener('message', function (e) {
    var data = e.data;
    if (!data || data.type !== 'lifeops-inspector') return;
